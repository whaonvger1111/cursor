...
! ============================================================
    ...
    subroutine step2_liquidation(val_c, val0_c, profit_mat, k_grid, b_grid, theta, delta, nk, nb, nx)
        ...
        !$omp parallel do private(x_c, b_c, k_c, k_val, b_val, profit_val, liq_val)
        do x_c = 1, nx
            do b_c = 1, nb
                do k_c = 1, nk
                    k_val = k_grid(k_c)
    ...
    subroutine step3_constraint(val0_c, val0_u, b_hat, b_grid, val0, is_c, nk, nb, nx)
        ...
        !$omp parallel do private(x_c, b_c, k_c, b_val)
        do x_c = 1, nx
            do b_c = 1, nb
                do k_c = 1, nk
                    b_val = b_grid(k_c, b_c)
    ...
    subroutine sub_vfi_onestep_fortran(val_c, val0_u, kp_bar, B_hat, profit_mat, k_grid, b_grid, &
                                       pi_x, theta, q, delta, psi, val_c_new, pol_kp_ind, nk, nb, nx)
        ! 约束企业VFI的单步Bellman算子
        ! 完整实现：STEP 2 + STEP 3 + STEP 4
        integer, intent(in) :: nk, nb, nx
        real(kind=8), dimension(nk, nb, nx), intent(in) :: val_c, val0_u, kp_bar
        real(kind=8), dimension(nk, nx), intent(in) :: B_hat, profit_mat
        real(kind=8), dimension(nk), intent(in) :: k_grid
        real(kind=8), dimension(nk, nb), intent(in) :: b_grid
        real(kind=8), dimension(nx, nx), intent(in) :: pi_x
        real(kind=8), intent(in) :: theta, q, delta, psi
        real(kind=8), dimension(nk, nb, nx), intent(out) :: val_c_new
        integer, dimension(nk, nb, nx), intent(out) :: pol_kp_ind
        
        ! 中间变量
        real(kind=8), dimension(nk, nb, nx) :: val0_c, val0
        integer, dimension(nk, nb, nx) :: is_c
        real(kind=8), dimension(nk, nb) :: EVx
        real(kind=8), dimension(nk) :: rhs_vec
        integer :: k_c, b_c, x_c, xp_c, kp_c, kp_ub_ind, max_ind
        real(kind=8) :: k_val, b_val, profit_val, kp_val, kp_ub, bprime, v0_int
        real(kind=8) :: adj_val, rhs_val, max_val
        integer :: jl
        real(kind=8) :: omega
        integer :: left, right, mid  ! 二分查找变量
        
        ! STEP 2 - 施加清算，方程(24)
        call step2_liquidation(val_c, val0_c, profit_mat, k_grid, b_grid, theta, delta, nk, nb, nx)
        
        ! STEP 3 - 执行方程(23)
        call step3_constraint(val0_c, val0_u, B_hat, b_grid, val0, is_c, nk, nb, nx)
        ...
        !$omp parallel do private(x_c, EVx, xp_c, b_c, k_c, k_val, b_val, profit_val, kp_ub, &
        !$omp& kp_ub_ind, kp_c, rhs_vec, kp_val, adj_val, bprime, jl, omega, &
        !$omp& v0_int, rhs_val, max_val, max_ind, left, right, mid) schedule(static)
        do x_c = 1, nx
            ! 计算方程(25)中的期望值 EVx(k',b')
            EVx = 0.0d0
            do xp_c = 1, nx
            ...
            do b_c = 1, nb
                do k_c = 1, nk
                    ...
                    ! 找到kp_ub在k_grid中的位置（使用二分查找优化）
                    ! 二分查找：找到最大的kp_c使得k_grid(kp_c) <= kp_ub
                    if (k_grid(nk) <= kp_ub) then
                        kp_ub_ind = nk - 1
                    else if (k_grid(1) > kp_ub) then
                        kp_ub_ind = 0
                    else
                        ! 二分查找
                        left = 1
                    ...
                    kp_ub_ind = min(max(kp_ub_ind, 0), nk - 1)
                    
                    ! 检查是否约束且非强制清算
                    if (is_c(k_c, b_c, x_c) == 1 .and. &
                        profit_val - b_val + theta * (1.0d0 - delta) * k_val >= 0.0d0) then
                        ...
                        do kp_c = 1, kp_ub_ind + 1
                            ...
                            ! 计算调整成本
                            adj_val = kp_val - (1.0d0 - delta) * k_val
                            if (kp_val < (1.0d0 - delta) * k_val) then
                                adj_val = theta * adj_val
                            end if
                            
                            ! 计算bprime
                            bprime = max(b_grid(kp_c, 1), (1.0d0 / q) * (b_val - profit_val + adj_val))
                            
                            ! 插值EVx(kp_c, bprime)
                            call myfind_loc_fortran(b_grid(kp_c, :), nb, bprime, jl, omega)
                            jl = max(0, min(jl, nb - 2))
                            v0_int = omega * EVx(kp_c, jl + 1) + (1.0d0 - omega) * EVx(kp_c, min(jl + 2, nb))
                            
                            ! 计算RHS
                            rhs_val = q * (psi * (theta * (1.0d0 - delta) * kp_val - bprime) + &
                                          (1.0d0 - psi) * v0_int)
                            rhs_vec(kp_c) = rhs_val
                        end do
                        ...
                        val_c_new(k_c, b_c, x_c) = max_val
                    else
                        ! 企业无约束或强制清算：val_c无关
                        val_c_new(k_c, b_c, x_c) = theta * (1.0d0 - delta) * k_val - b_val
                        pol_kp_ind(k_c, b_c, x_c) = 0
                    end if
                end do
            end do
        end do
        !$omp end parallel do
    end subroutine sub_vfi_onestep_fortran
...
! ============================================================
    ...
    subroutine sub_mu_onestep_fortran(mu, phi_dist, pol_kp_ind, pol_exit, pol_entry, &
        ...
        ! 更新分布（并行化x_c循环，每个线程处理不同的x_c，无竞争条件）
        !$omp parallel do private(x_c, b_c, k_c, dexit, entry, knext_ind, left_loc, omega)
        do x_c = 1, nx
            do b_c = 1, nb
                do k_c = 1, nk
        ...
        ! 矩阵乘法: mu1(k',b',x)*pi(x,x')==> mu1(k',b',x')（并行化k_c循环）
        !$omp parallel do private(k_c, temp, xp_c, x_c)
        do k_c = 1, nk
            temp = mu1(k_c, :, :)
            mu1(k_c, :, :) = 0.0d0
            do xp_c = 1, nx
